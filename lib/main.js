...
HelpFile.prototype.end = function() {
  ...
  if (data.sections[0] && data.sections[0].depth == 1) {
    data.header = data.sections.shift();
  }
...
Tokens.prototype.heading = function(t) {
  ...
  function format_line(a, b, len, head_indent, tail_indent) {
    ...
    var n = len - (head_indent + tail_indent + a.length + b.length + 1 + 1)
      , sp = new Array(n < 0 ? 0 : n).join(" ")
  ...
  this.last = s[s.length] = {
    ...
    index_title: format_line(t.text, "|" + id + "|", 68, (t.depth - 1) * 2),
